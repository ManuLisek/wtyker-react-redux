export const getFilteredProducts = ({products, filters}) => {
  ...
  //filter by tags
  output = output.filter(product => filters.checkedTags.every(tag => product.tags.indexOf(tag) > -1));

  //filter by brands
  output = output.filter(product => filters.checkedBrands.every(brand => product.brand.indexOf(brand)  > -1));
